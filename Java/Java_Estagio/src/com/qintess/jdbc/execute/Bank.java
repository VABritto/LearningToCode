...
public class Bank {
	...
	private static void alter(Venda venda, Livro livro) throws SQLException {
		...
		case "1": {
			...
			String sql = "UPDATE itens_de_vendas "
					+ "SET idlivro = ? AND subtotal = ? "
					+ "WHERE idvenda = ? AND idlivro = ?";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, novoLivro.getIdlivro());
			float subtotal = novoLivro.getPreco() * itemVenda.getQtd();
	...
	private static void insert(Venda venda, Livro livro) throws SQLException {
		...
		String sql = "INSERT INTO itens_de_vendas (idvenda, idlivro, qtd, "
				+ "subtotal) VALUES (?, ?, ?, ?)";
		PreparedStatement ps = conn.prepareStatement(sql);
		ps.setInt(1, venda.getIdvenda());
		ps.setInt(2, livro.getIdlivro());
		ps.setInt(3, qnt);
		float subtotal = qnt * livro.getPreco();
	...
	private static Livro getLivro(String titulo, String nomeAutor) throws SQLException {
		...
		while (rAutores.next()) {
			Autor autor = new Autor(rAutores.getInt("idautor"), 
					rAutores.getString("nome"));
			autores.add(autor);
		}
		...
		for (Autor a: autores) {
			for (Livro l: livros) {
				...
				while (rEscreve.next()) {
					Escreve escreve = new Escreve(rEscreve.getInt("idlivro"), 
							rEscreve.getInt("idautor"));
					if (escreve != null) {
		...
		while (rLivro.next()) {
			Livro livroEscolhido = new Livro(rLivro.getInt("idlivro"), 
					rLivro.getString("titulo"), rLivro.getFloat("preco"));
